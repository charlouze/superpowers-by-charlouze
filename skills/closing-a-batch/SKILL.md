...
- **Create the branch and its workspace by invoking `superpowers:using-git-worktrees`.** The conventional name is `batch/NN-<slug>-close`, enforced by this plugin, not by that skill. If it lands on a differently named branch or a detached HEAD, restore the conventional name before going on. **A named branch is not enough** — and here, as in adoption, that is convention rather than mechanism: nothing reads `batch/*-close`, and this batch's `NN` is already held by `docs/batches/NN-<slug>/` on `main`, since closing only runs on a batch whose opening pull request merged. The convention is uniform because one honoured only where a scan would catch you is not a convention at all.
